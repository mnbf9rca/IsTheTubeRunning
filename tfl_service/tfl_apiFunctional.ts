...
import query_cache from './cache'


const structure_cached_value = (cached_value: any, cache_ttl: number | undefined) => {
  /**
...
export async function get_line_stoppoints(line: Line.Line) {
  /**
   ...
   *
   */
  const cache_key = `line_stoppoints-${line.lineName}`
  const cached_value = query_cache.get(cache_key)
  if (cached_value) {
    logger.debug(`${cache_key} cache hit`)
    return structure_cached_value(cached_value, query_cache.getTtl(cache_key))
...
function extract_stoppoints_from_stoppoint_array(stoppoint_array: TfL_Types['StopPoint'][] | TfL_Types['MatchedStop'][]) {
...
function returnFirstMatchingProperty(object: any, properties: string[]) {
